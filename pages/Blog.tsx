...
const Blog: React.FC<BlogProps> = ({ onNavigate, onBack, initialCategory = 'Semua' }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-8">
        <button onClick={onBack} className="text-slate-500 hover:text-slate-900 font-medium flex items-center gap-2 mb-4 transition-colors">
          &larr; Kembali
        </button>
              ...
              <button
                ...
                className={`px-5 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-all border ${
                  selectedCategory === cat
                    ...
                    : 'bg-white text-slate-600 border-slate-300 hover:bg-slate-100 hover:text-slate-900 hover:border-blue-200'
                }`}
              >
                {cat}
              </button>
          ...
          {filteredPosts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredPosts.map(post => (
                <div 
                  key={post.id} 
                  className="bg-white rounded-xl overflow-hidden border border-slate-200 shadow-sm hover:shadow-lg hover:border-slate-300 transition-all cursor-pointer group flex flex-col h-full"
                ...
                >
                  <div className="aspect-[16/9] w-full overflow-hidden bg-slate-100 relative">
                    ...
                    <div className="absolute top-4 left-4">
                      <span className="bg-blue-600/90 backdrop-blur text-white text-xs font-bold px-3 py-1 rounded-full shadow-md">
                        {post.category}
                      </span>
                    </div>
                  </div>
                  <div className="p-6 flex flex-col flex-grow">
                    <div className="text-xs text-slate-500 font-bold mb-2 uppercase tracking-wide">
                    ...
                    <div className="text-blue-700 font-bold text-sm flex items-center gap-1 group-hover:gap-2 transition-all">
